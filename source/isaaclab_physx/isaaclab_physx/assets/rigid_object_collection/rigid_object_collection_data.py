...
class RigidObjectCollectionData(BaseRigidObjectCollectionData):
    ...
    def projected_gravity_b(self) -> wp.array:
        ...
        if self._projected_gravity_b.timestamp < self._sim_timestamp:
            wp.launch(
                quat_apply_inverse_2D_kernel,
                dim=(self.num_instances, self.num_bodies),
                inputs=[self.GRAVITY_VEC_W, self.body_link_quat_w, self._projected_gravity_b.data],
                device=self.device,
            )
    ...
    def heading_w(self) -> wp.array:
        ...
        if self._heading_w.timestamp < self._sim_timestamp:
            wp.launch(
                body_heading_w,
                dim=(self.num_instances, self.num_bodies),
                inputs=[self.FORWARD_VEC_B, self.body_link_quat_w, self._heading_w.data],
                device=self.device,
            )
    ...
    def body_link_lin_vel_b(self) -> wp.array:
        ...
        if self._body_link_lin_vel_b.timestamp < self._sim_timestamp:
            wp.launch(
                quat_apply_inverse_2D_kernel,
                dim=(self.num_instances, self.num_bodies),
                inputs=[self.body_link_lin_vel_w, self.body_link_quat_w, self._body_link_lin_vel_b.data],
                device=self.device,
            )
    ...
    def body_link_ang_vel_b(self) -> wp.array:
        ...
        if self._body_link_ang_vel_b.timestamp < self._sim_timestamp:
            wp.launch(
                quat_apply_inverse_2D_kernel,
                dim=(self.num_instances, self.num_bodies),
                inputs=[self.body_link_ang_vel_w, self.body_link_quat_w, self._body_link_ang_vel_b.data],
                device=self.device,
            )
    ...
    def body_com_lin_vel_b(self) -> wp.array:
        ...
        if self._body_com_lin_vel_b.timestamp < self._sim_timestamp:
            wp.launch(
                quat_apply_inverse_2D_kernel,
                dim=(self.num_instances, self.num_bodies),
                inputs=[self.body_com_lin_vel_w, self.body_link_quat_w, self._body_com_lin_vel_b.data],
                device=self.device,
            )
    ...
    def body_com_ang_vel_b(self) -> wp.array:
        ...
        if self._body_com_ang_vel_b.timestamp < self._sim_timestamp:
            wp.launch(
                quat_apply_inverse_2D_kernel,
                dim=(self.num_instances, self.num_bodies),
                inputs=[self.body_com_ang_vel_w, self.body_link_quat_w, self._body_com_ang_vel_b.data],
                device=self.device,
            )
    ...
    def _create_buffers(self) -> None:
        ...
        self._body_state_w = TimestampedBuffer((self.num_instances, self.num_bodies), self.device, vec13f)
        self._body_link_state_w = TimestampedBuffer((self.num_instances, self.num_bodies), self.device, vec13f)
