...
#     with SimulationContext.use_stage(context_stage):
...
def get_current_stage(fabric: bool = False) -> "Usd.Stage | usdrt.Usd._Usd.Stage":
    ...
    # First check if SimulationContext has a stage set (via use_stage context manager)
    stage = SimulationContext.get_stage()

    # If no stage set in SimulationContext, fall back to USD context
    if stage is None:
        stage = Usd.Stage.CreateInMemory()
        SimulationContext.set_stage(stage)
